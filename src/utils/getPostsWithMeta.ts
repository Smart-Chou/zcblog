...
function generateExcerptFromMarkdown(markdownContent: string): string {
    ...
    try {
        ...
        // 清理HTML，只保留安全的标签
        const allowedTags = new Set(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'ul', 'ol', 'li']);
        ...
        // 4. 提取有意义的文本内容
        // 优先获取段落和标题内容
        const meaningfulElements = document.querySelectorAll('p, h1, h2, h3, h4, h5, h6');
        let textContent = '';
