...
async function preloadImageDimensions(
  ...
  items.forEach((item, index) => {
    ...
    if (img.complete && img.naturalWidth > 0) {
      ...
      if (debug) console.log(`图片 ${index} 尺寸: ${img.naturalWidth}x${img.naturalHeight} (缓存)`);
      return;
    }
    ...
    const promise = new Promise<void>((resolve) => {
      ...
      loader.src = img.src;
      loader.onload = () => {
        item.setAttribute('data-pswp-width', loader.naturalWidth.toString());
        item.setAttribute('data-pswp-height', loader.naturalHeight.toString());
        if (debug) console.log(`图片 ${index} 尺寸: ${loader.naturalWidth}x${loader.naturalHeight} (加载)`);
        resolve();
      };
      loader.onerror = () => {
        // 加载失败时使用默认尺寸
...
export async function initDynamicPhotoSwipe(
  ...
  try {
    ...
    // 查找所有图片
    const images = content.querySelectorAll<HTMLImageElement>(config.imageSelector || 'img');
    if (images.length === 0) {
    ...
    if (debug) console.log(`PhotoSwipe lightbox 初始化完成，图片数量: ${imageData.length}`);
