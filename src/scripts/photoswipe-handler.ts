...
function initPhotoSwipeForContentImages() {
    ...
    try {
        ...
        const loadPhotoSwipeCSS = () => {
            ...
            return import("photoswipe/style.css")
                .then(() => {
                    console.log("PhotoSwipe CSS 加载成功");
                })
                .catch((error) => {
                    console.warn(
                        "动态加载 PhotoSwipe CSS 失败，尝试使用 CDN 回退",
                        error,
                    );
                    // CDN 回退
        ...
        Promise.all([loadPhotoSwipeCSS(), import("photoswipe/lightbox")])
            .then(([, { default: PhotoSwipeLightbox }]) => {
                ...
                console.log(
                    "PhotoSwipe lightbox 初始化完成，图片数量:",
                    imageData.length,
                );
                ...
                lightbox.on("loadComplete", (e: any) => {
                    console.log(
                        `图片 ${e.index + 1}/${imageData.length} 加载完成`,
                    );
                });
